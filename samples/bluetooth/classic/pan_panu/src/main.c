...
#define NVDS_FLASH_OFFSET DT_REG_ADDR(DT_NODELABEL(nvds_partition))
...
/* Discovery/retry pacing. RECONNECT_DELAY also gives a previous ACL enough
 * time to fully tear down before we page the peer again.
 */
...
/*
 * IEEE 802 first-octet checks on a BD_ADDR/OUI:
 *   bit0 (0x01) = Individual/Group bit - must be 0 for a normal unicast
 *                 station address.
 *   bit1 (0x02) = Universal/Local bit  - must be 0 for an IEEE-assigned
 *                 (burned-in) OUI; if set, the address is locally
 *                 administered (made up), which is what the platform's
 *                 `nvds get_mac` flags as an invalid/illegal address.
 * Example: a valid OUI E8:6A:64 has first octet 0xE8 = 0b1110_1000, so both
 * bits are 0 and it passes.
 */
...
static void send_ping(struct k_work *work)
{
	...
	if (ping_sent_count == PING_CHECK_COUNT) {
		...
		/* A PAN link is not a successful autoconnect until the network is
		 * usable. Tear it down so disconnected() schedules the next attempt.
		 */
		if (reconnect_active) {
			conn = bt_pan_get_conn(active_pan);
			if (conn != NULL) {
...
static void pan_connected(struct bt_pan *pan)
{
	...
	/* Keep autoconnect active until DHCP and the three-ping network check
	 * succeed. A phone may accept BNEP briefly even when network sharing is
	 * disabled, then disconnect; that must consume one attempt and retry.
	 */
	active_pan = pan;
	ping_sent_count = 0U;
	ping_reply_count = 0U;
...
static void pan_disconnected(struct bt_pan *pan)
{
	...
	/*
	 * The PAN session went down (usually because the peer rejected the
	 * BNEP connection). Drop the ACL so the discovery/connect loop can
	 * start over instead of sitting on a dead link.
	 */
...
static void connected(struct bt_conn *conn, uint8_t err)
{
	...
	/*
	 * Pair/bond before starting PAN. Phones that share their network over
	 * Bluetooth usually reject an unpaired PANU at the BNEP level, which is
	 * why the very first PAN connect failed. The PAN session is started
	 * from security_changed() once the link is encrypted; if a bond already
	 * exists this connection is already secure and we can start right away.
	 */
...
static void disconnected(struct bt_conn *conn, uint8_t reason)
{
	...
	if (reconnect_active) {
		...
		/*
		 * Give the previous link time to fully tear down before paging the
		 * peer again, otherwise bt_conn_create_br() fails because a connection
		 * to that address already exists.
		 */
		(void)k_work_schedule(&discover_work, RECONNECT_DELAY);
	}
}
...
static void discovery_timeout_cb(const struct bt_br_discovery_result *results, size_t count)
{
	...
	/*
	 * List every device found and let the user pick one with
	 * 'pan connect <index>' instead of blindly connecting to the first
	 * NAP-like device seen (which may be useless and wastes time).
	 */
	for (size_t i = 0; i < scan_count; i++) {
		...
		if (nap) {
			/* Remember the last NAP-capable peer for 'pan autoconnect'. */
...
static void discover_work_handler(struct k_work *work)
{
	...
	/* Never scan while a connection is up or being set up. */
...
/* ---- pan status ---------------------------------------------------------
 * Print ACL / PAN session / IP status, for quick sanity checks from the
 * console instead of scrolling back through the log.
 * -------------------------------------------------------------------------- */
...
/* ---- pan scan -------------------------------------------------------------
 * Run a BR/EDR scan and print the list of devices found (Device[0..n]). It no
 * longer auto-connects: pick one with 'pan connect <index>'. No-op (with a
 * warning) while an ACL is already up or being negotiated.
 * -------------------------------------------------------------------------- */
...
/* ---- pan connect ----------------------------------------------------------
 * Connect to one of the devices reported by the last scan, by its index.
 * -------------------------------------------------------------------------- */
static int cmd_pan_connect(const struct shell *sh, size_t argc, char **argv)
{
	...
	/* Keep 'pan autoconnect' pointing at the phone when the selected device
	 * is PAN-capable.
	 */
...
/* ---- pan disconnect -------------------------------------------------------
 * Manually drop the current ACL (and therefore the PAN session riding on
 * it). connected()/disconnected() take care of restarting discovery.
 * -------------------------------------------------------------------------- */
...
/* ---- pan ping --------------------------------------------------------------
 * Fire one ICMP echo request immediately instead of waiting for the
 * periodic send_ping() timer.
 * -------------------------------------------------------------------------- */
...
static int cmd_pan_set_retry_flag(const struct shell *sh, size_t argc, char **argv)
{
	...
	/* shell_strtoul() leaves err untouched on success, so it must be
	 * initialized to zero before the call.
	 */
...
/* ---- pan mac / nvds --------------------------------------------------------
 * `pan mac get` and `nvds get_mac` read the controller's public BD_ADDR and
 * validate its OUI. `nvds update addr 6 <arg>` stores an NVDS tag which is
 * injected before the LCPU starts on the next reboot. The input uses the
 * SiFli SDK encoding: three custom bytes followed by the reversed OUI
 * (e.g. ABCDEF646AE8 becomes E8:6A:64:EF:CD:AB).
 *
 * `pan mac oui_reverse <OUI> [suffix]` builds that encoded argument.
 *
 * An invalid (locally-administered) OUI on the controller is a common cause
 * of otherwise-unexplained network weirdness once PAN comes up (DHCP/ARP
 * oddities, `pan weather` failing to reach the internet, etc.) on phones
 * that are picky about the peer's Bluetooth identity. If a manual
 * `nvds update addr 6 ...` doesn't survive a reboot, check whether
 * CONFIG_NVDS_AUTO_UPDATE_MAC_ADDRESS is enabled -- it overwrites the
 * address on every boot and will stomp on a manual change.
 * -------------------------------------------------------------------------- */
...
static int cmd_pan_mac_get(const struct shell *sh, size_t argc, char **argv)
{
	...
	/* bt_addr_t.val[] is little-endian: val[5] is the OUI's first octet. */
	a = addr.val;
	...
	if (nvds_record_read(&record) == 0) {
		...
		shell_print(sh, "NVDS    : %02X:%02X:%02X:%02X:%02X:%02X%s",
			    ...
			    memcmp(a, addr.val, NVDS_BD_ADDRESS_LEN) == 0 ? "" : " (reboot pending)");
	}
	...
	if (bd_addr_oui_is_valid(a[5])) {
		...
		shell_warn(sh, "OUI     : INVALID (locally-administered and/or multicast "
			   "bit set on %02X)", a[5]);
		shell_warn(sh, "Fix with the platform command:");
		shell_warn(sh, "  nvds update addr 6 <your-3-byte-suffix><reversed-OUI>");
		shell_warn(sh, "Use 'pan mac oui_reverse <OUI> [suffix]' to build that "
			   "string for you.");
		shell_warn(sh, "If it reverts after reboot, check whether "
			   "CONFIG_NVDS_AUTO_UPDATE_MAC_ADDRESS is enabled and disable it.");
	}

	return 0;
}

/* Parse a hex byte string like "E86A64" or "E8:6A:64" into out_len raw bytes.
 * Accepts ':' and '-' as optional separators, ignored during parsing.
 */
...
static int cmd_nvds_update(const struct shell *sh, size_t argc, char **argv)
{
	...
	shell_warn(sh, "reboot required; then run 'nvds get_mac' to verify the active address");
	shell_warn(sh, "this Zephyr port has no CONFIG_NVDS_AUTO_UPDATE_MAC_ADDRESS option");
...
/* ---- pan weather -----------------------------------------------------------
 * Fetch the current weather over the PAN link:
 *   1. Resolve WEATHER_HOST via DNS,
 *   2. Open a TCP socket and send a plain HTTP/1.1 GET request,
 *   3. Parse the JSON body with Zephyr's builtin JSON library,
 *   4. Print the result.
 * Host/key/city/language are defined above with the other sample constants.
 * -------------------------------------------------------------------------- */
...
/* Static buffers: `pan weather` is not meant to run concurrently from
 * multiple shell sessions, so we avoid extra stack usage / heap allocation.
 */
			    ...
			    size_t out_size)
{
	...
	/* We asked for "Connection: close", so read until the peer closes the
	 * socket (recv() returns 0) or our buffer fills up.
	 */
